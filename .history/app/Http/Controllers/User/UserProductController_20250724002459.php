...
class UserProductController extends Controller
{
    ...
    public function addToCart(Request $request)
    {
        ...
        dd($request->all());
$attributeValues = collect($request->input('attribute'))->values()->toArray(); // [5, 8]

$product = Product::where('id', 23)
    ->whereHas('variants', function ($variantQuery) use ($attributeValues) {
        $variantQuery->whereHas('variantValues', function ($valueQuery) use ($attributeValues) {
            $valueQuery->whereIn('id', $attributeValues);
        }, '=', count($attributeValues)); // Đảm bảo có đủ tất cả giá trị
    })
    ->with(['variants.variantValues']) // Eager load để sau xài
    ->first();
