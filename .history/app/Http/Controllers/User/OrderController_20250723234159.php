...
class OrderController extends Controller
{
    public function index(Request $request)
    {
        ...
        // Eager load toàn bộ variant_value 1 lần cho tất cả cartDetails
        $allVariantValueIds = $items->flatMap(function ($item) {
            return explode(',', $item->product_variant_id);
        })->unique()->filter();

        // Truy vấn duy nhất 1 lần để lấy các giá trị cần thiết
        $variantValues = AttributeValue::with('attribute')
            ->whereIn('id', $allVariantValueIds)
            ->get()
            ->keyBy('id'); // key theo ID để tra nhanh

        // Gắn vào từng cart detail
        $items->each(function ($item) use ($variantValues) {
            $ids = explode(',', $item->product_variant_id);

            $item->variant_summary = collect($ids)->reduce(function ($carry, $id) use ($variantValues) {
                $variant = $variantValues[$id] ?? null;

                if (!$variant || !$variant->attribute) return $carry;

                $carry[$variant->attribute->name] = $variant->value;
